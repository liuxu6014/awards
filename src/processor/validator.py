...
class DataValidator:
    ...
    def validate_award_data(self, data: Dict[str, Any]) -> Tuple[bool, Dict[str, List[str]]]:
        """
        验证奖项数据
        ...
        # 验证基本字段
        is_valid, field_errors = self.validate_text(data.get('title', ''), '标题')
        if not is_valid:
            errors['title'] = field_errors
        
        is_valid, field_errors = self.validate_text(data.get('content', ''), '内容')
        if not is_valid:
            errors['content'] = field_errors
        
        is_valid, field_errors = self.validate_url(data.get('source_url', ''))
        if not is_valid:
            errors['source_url'] = field_errors
        
        is_valid, field_errors = self.validate_year(data.get('year'))
        if not is_valid:
            errors['year'] = field_errors
        
        is_valid, field_errors = self.validate_award_level(data.get('award_level'))
        if not is_valid:
            errors['award_level'] = field_errors
        
        is_valid, field_errors = self.validate_award_type(data.get('award_type'))
        if not is_valid:
            errors['award_type'] = field_errors
        
        # 验证项目列表
        projects = data.get('projects', [])
        if not projects:
            errors['projects'] = ["项目列表不能为空"]
        else:
            project_errors = []
            for i, project in enumerate(projects):
                is_valid, proj_errors = self.validate_project(project)
                if not is_valid:
    ...
    def validate_project(self, project: Dict[str, Any]) -> Tuple[bool, Dict[str, List[str]]]:
        """
        验证项目数据
        ...
        # 验证项目名称
        is_valid, field_errors = self.validate_text(project.get('name', ''), '项目名称')
        if not is_valid:
            errors['name'] = field_errors
        
        # 验证获奖人列表
        winners = project.get('winners', [])
        if not winners:
            errors['winners'] = ["获奖人列表不能为空"]
        else:
            winner_errors = []
            for i, winner in enumerate(winners):
                is_valid, win_errors = self.validate_winner(winner)
                if not is_valid:
            ...
            if winner_errors:
                errors['winners'] = winner_errors
        
        return len(errors) == 0, errors
    
    def validate_winner(self, winner: Dict[str, Any]) -> Tuple[bool, Dict[str, List[str]]]:
        """
        验证获奖人数据
        ...
        # 验证姓名
        is_valid, field_errors = self.validate_text(winner.get('name', ''), '获奖人姓名', min_length=2)
        if not is_valid:
            errors['name'] = field_errors
        
        # 验证机构名称（可选）
        org = winner.get('organization')
        if org:
            is_valid, field_errors = self.validate_text(org, '机构名称', min_length=4)
            if not is_valid:
                errors['organization'] = field_errors
